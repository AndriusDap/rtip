...
export class EventsService {
    ...
    public getStationsList(): Promise<EventCityList[]> {

        var deferred = this.$q.defer();

        // simulating hitting server
        this.$timeout(() => {
            deferred.resolve(this.stations);
        }, 2000);

        return deferred.promise;
    }

    public getStationDetail(query): Promise<EventCity> {

        var deferred = this.$q.defer();

        this.$timeout(() => {
            for(var i=0; i<this.stations.length; i++) {
                if(angular.lowercase(this.stations[i].city) == angular.lowercase(query)) {
                    deferred.resolve(this.stations[i]);
                    return;
                }
            }
            deferred.reject();
